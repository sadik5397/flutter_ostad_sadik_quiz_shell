...
class BdappsService {
  ...
  static const Map<String, String> _headers = {'Content-Type': 'application/x-www-form-urlencoded'};

  // --- OTP flows -----------------------------------------------------------

  static Future<http.Response> sendOtp(String mobile) {
    return http.post(Uri.parse('$_base/send_otp.php'), headers: _headers, body: {'user_mobile': mobile});
  }

  static Future<http.Response> verifyOtp(String otp, String referenceNo) {
    return http.post(Uri.parse('$_base/verify_otp.php'), headers: _headers, body: {'Otp': otp, 'referenceNo': referenceNo});
  }
  ...
  static Future<http.Response> checkSubscription(String mobile) {
    return http.post(Uri.parse('$_base/check_subscription.php'), headers: _headers, body: {'user_mobile': mobile});
  }

  static Future<http.Response> unsubscribe(String mobile) {
    return http.post(Uri.parse('$_base/unsubscribe.php'), headers: _headers, body: {'user_mobile': mobile});
  }
      ...
      try {
        ...
        if (decoded is Map) {
          ...
          if (statusDetail is String) {
            ...
            if (lower.contains('unregistered') || lower.contains('not registered') || lower.contains('not subscribed') || lower.contains('forbidden') || lower.contains('forid')) {
              return false;
            }
          ...
          for (final key in const ['status', 'registered', 'isSubscribed', 'subscribed', 'state']) {
            final value = decoded[key];
            if (value is bool) return value;
            if (value is num) return value != 0;
            if (value is String) {
              final lower = value.toLowerCase();
              if (lower == 'true' || lower == 'yes' || lower == 'active' || lower == 'registered') {
                return true;
              }
              if (lower == 'false' || lower == 'no' || lower == 'inactive' || lower == 'unregistered') {
                return false;
              }
            }
          }
